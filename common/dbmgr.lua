...
--[[
	covert record number
	pramrams : table_name ,record
	return   : record
]]
...
function CMD.start(config, user, common)
	...
	-- end
	-- load mysql schema to redis
	-- load_schema_to_redis()
	-- load data to redis
	-- load_data_to_redis()

	-- local result =skynet.call(mysqlpool,"lua","callProc","get_user_info",{10000},false)
	-- print(" call_proc result ",result)
	-- local result1 = skynet.call(mysqlpool,"lua","execute","select * from user_info",false)
	-- print(" execute result   ",result1)
	-- local result2 = skynet.call(mysqlpool,"lua","callProcTest")
